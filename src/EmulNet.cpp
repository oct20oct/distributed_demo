...
int EmulNet::ENrecv(Address *myaddr, int (* enq)(void *, char *, int), struct timeval *t, int times, void *queue){
	...
	for( i = emulnet.currbuffsize - 1; i >= 0; i-- ) {
		...
		if ( 0 == strcmp(emsg->to.addr, myaddr->addr) ) {
			...
			//debug code, printing out all the address stored in msg
			//cout<<"testing msg decode.."<<emsg->to.addr<<endl;
			//std::stringstream ss;
			//ss << static_cast<const void*>(tmp);
			//string pointer_address_string = ss.str();
			//cout<<"tmp stored addr.. "<<pointer_address_string<<endl;
			//ss << static_cast<const void*>(myaddr->addr);
			//string q=ss.str();
			//cout<<"my current addr.."<<q<<endl;
			//
